 ...
 * 由于超过 ⌊ n/2 ⌋ 次，可以对上面的方法改进一下，排序后，不需要扫描，直接返回中间那个元
 * 素， nums[n/2] ，肯定就是答案。
 * 上述两个方法都是 O(nlogn) 的，本题有一个线性解法。由于超过 ⌊ n/2 ⌋ ，可以用相抵消的思想，凡
 * 是和最多元素不相等的，就抵消，最后剩下的一定就是最多的那个元素。
 *
 * @author GreePanda
 * @version 1.0
 * @date 2020/5/8 10:29
...
public class MajorityElement {
    public static void main(String[] args) {
        ...
        final int[] nums = {-1, 3, -2, 11, 0, 100, 45};
        int ret = majority(nums);
        int ret1 = majority1(nums);
